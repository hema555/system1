...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['ahelp|| 16/6/2019 soon'];
    var i = -1;
    var j = 0;
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'SSY Community ©\'s translator',
...
client.users.get("248367501669433354").send(
    "\n" + "**" + "? السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ? المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
.setDescription(``)
if (!args) return message.reply('**يجب عليك كتابة كلمة او جملة**');message.channel.send(BcList).then(msg => {
msg.react('')
.then(() => msg.react('::'))
.then(() =>msg.react(''))
 
let EmbedBcFilter = (reaction, user) => reaction.emoji.name === 'radioactive' && user.id === message.author.id;
let NormalBcFilter = (reaction, user) => reaction.emoji.name === 'heart' && user.id === message.author.id;
...
client.on('guildDelete', guild => {
     ...
     .setURL('')
  .setDescription(`**
  Server Kicked SSY Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
